...
describeFlowIntegration(
  'FlowInvoices - Pruebas de integración con sandbox',
  () => {
    ...
    it('Cancelar un Invoice pendiente de pago (cancelPendingPayment)', async () => {
      ...
      try {
        const cancelResponse = await flowInvoices.cancelPendingPayment(
          TEST_INVOICE_ID!,
        );
    ...
    it('Reintentar cobro de un Invoice vencido (retryOverdueInvoicePayment)', async () => {
      ...
      try {
        const retryResponse = await flowInvoices.retryOverdueInvoicePayment(
          OVERDUE_INVOICE_ID!,
        );
